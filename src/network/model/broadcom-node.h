...
namespace ns3 {
	...
	public:
		...
		static const unsigned pCnt = 13; //33 ;//  33;//64; port 0 is not used so + 1	// Number of ports used
	...
	private:
		...
		uint32_t stat_max_usedEgressPortBytes [pCnt] {0,};
		uint32_t stat_max_bytesEgressTltUip  [pCnt] {0,};

		uint64_t stat_sum_usedEgressPortBytes [pCnt] {0,};
		uint64_t stat_sum_usedEgressTltUip [pCnt] {0,};
		uint64_t stat_sum_usedEgressPortAccCount [pCnt] {0, };
		bool stat_sum_queueDirty [pCnt] {0, };
		bool stat_queuestat_scheduled {false};
