 ...
 */
	...
	public:
		...
		static const uint32_t fCnt = 250; // Max number of flows on a NIC, for TX and RX respectively. TX+RX=fCnt*2
		...
		virtual bool Attach(Ptr<QbbChannel> ch);
		...
		virtual void SetQueue(Ptr<BEgressQueue> q);
		virtual Ptr<BEgressQueue> GetQueue();
	...
	protected:
		...
		Time m_pauseStart[qCnt];
		Time m_pauseDuration[qCnt];
		//qcn
		...
		void RetransmitIRN(uint32_t seq, uint32_t i); // Handles IRN Retransmission on timeout
		uint32_t GetPacketsInflight(uint32_t i);
	   ...
	   // void ProcessNACK(Ptr<Packet> packet);
		...
		uint32_t m_milestone_rx[fCnt];
		bool m_waitAck;
		bool m_backto0;
		bool m_testRead;
		uint32_t m_milestone_tx[fCnt];
		EventId m_retransmit[fCnt];
		bool m_waitingAck[fCnt];
		bool m_tltEnabled;
		bool m_PFCenabled;
		bool m_uselotofqueue;
		uint32_t m_tltPacketCount;
		uint32_t TLT_IMPORTANT_PKT_INTERVAL;
		bool m_irn;
		uint32_t m_irn_rtoLow;
		uint32_t m_irn_rtoHigh;
		uint32_t m_irn_bdp;
		std::queue<Ptr<Packet>> m_irn_unacked;
		std::deque<Ptr<Packet>> m_irn_retransmitted[fCnt];
		std::bitset<1024*16> m_irn_recv_sack[fCnt];

		uint32_t m_irn_retransmission_cnt = 0; //stat
		uint32_t m_irn_maxAck[fCnt];
		uint32_t m_irn_maxSeq[fCnt];
