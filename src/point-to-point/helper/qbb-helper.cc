...
QbbHelper::QbbHelper ()
{
  m_queueFactory.SetTypeId ("ns3::BEgressQueue");
  ...
  if (device == 0)
    {
      ...
      Ptr<PointToPointNetDevice> pppnd = nd->GetObject<PointToPointNetDevice> ();
      //NS_LOG_INFO ("QbbHelper::EnablePcapInternal(): Device is PPPND? " << pppnd);
      return;
    } else {
      NS_LOG_INFO ("QbbHelper::EnablePcapInternal(): Device is QbbNetDevice : is " << nd->GetTypeId ());
    }
...
{
  return Install(a, b, -1, -1);
}


NetDeviceContainer 
QbbHelper::Install (Ptr<Node> a, Ptr<Node> b, int a_id, int b_id)
{
  ...
  NS_ASSERT_MSG(devA && devB, "DevA or DevB NULL");
